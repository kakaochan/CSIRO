...
def get_decoder(cfg: DictConfig, n_channels: int, n_pairs: int, n_actions: int, num_timesteps: int) -> DECODERS:
    decoder: DECODERS
    n_classes = n_pairs * n_actions  # Calculate total classes
...
def get_model(cfg: DictConfig, feature_dim: int, n_pairs: int, n_actions: int, num_timesteps: int) -> MODELS:
    model: MODELS
    if cfg.model.name == "Spec2DCNN":
        feature_extractor = get_feature_extractor(cfg, feature_dim, num_timesteps)
        decoder = get_decoder(cfg, feature_extractor.height, n_pairs, n_actions, num_timesteps)
        model = Spec2DCNN(
            cfg=cfg,
        ...
        decoder = get_decoder(cfg, feature_extractor.height * 2, n_pairs * 2, n_actions, num_timesteps // 2)
        # model = Spec2DCNN2Day(
        #     cfg=cfg,
        #     feature_extractor=feature_extractor,
        #     decoder=decoder,
        ...
        feature_extractor = get_feature_extractor(cfg, feature_dim, num_timesteps // 2)
        decoder = get_decoder(cfg, feature_extractor.height * 2, n_pairs, n_actions, num_timesteps // 2)
        ...
        feature_extractor = get_feature_extractor(cfg, feature_dim, num_timesteps)
        decoder = get_decoder(
            cfg, feature_extractor.height * (cfg.model.n_split + 1), n_pairs, n_actions, num_timesteps // (2**cfg.model.n_split)
        )
        ...
        feature_extractor = get_feature_extractor(cfg, feature_dim, num_timesteps)
        decoder = get_decoder(cfg, 3 * feature_extractor.height, n_pairs, n_actions, num_timesteps)
        ...
        feature_extractor = get_feature_extractor(cfg, feature_dim, num_timesteps)
        decoders = [
            get_decoder(cfg, feature_extractor.height, 1, 1, num_timesteps),
            get_decoder(cfg, feature_extractor.height, 1, 1, num_timesteps),
        ]
        # model = SpecWeightAvg(
        #     cfg=cfg,
        ...
        feature_extractor = get_feature_extractor(cfg, feature_dim, num_timesteps)
        decoder = get_decoder(cfg, feature_extractor.height, 6, 1, num_timesteps)
        # model = CenterNet(
        #     feature_extractor=feature_extractor,
        #     decoder=decoder,
        #     in_channels=feature_extractor.out_chans,
